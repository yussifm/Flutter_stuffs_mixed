...
class StepNumber extends StatelessWidget {
  ...
  const StepNumber({key, required this.number}) : super(key: key);
...
class StepQuestion extends StatelessWidget {
  ...
  const StepQuestion({key, required this.question}) : super(key: key);
...
class OptionItem extends StatefulWidget {
  ...
  const OptionItem(
      {key, required this.name, required this.onTap, this.showDot = true})
      : super(key: key);
...
class ItemFader extends StatefulWidget {
  ...
  const ItemFader({key, required this.child}) : super(key: key);
    ...
    return AnimatedBuilder(
      ...
      builder: (context, child) {
        return Transform.translate(
          offset: Offset(0, 64 * position.toDouble() * (1 - _animation.value)),
          child: Opacity(
...
class Dot extends StatelessWidget {
  ...
  const Dot({key, this.visible = true}) : super(key: key);
